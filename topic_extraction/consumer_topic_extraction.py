...
reddit["query"] = reddit["topic"].map(topic_map)

# standardize columns so they can be concatenated
tweets_small = tweets[["tweet", "query"]].copy().rename(columns={"tweet": "text"})
reddit_small = reddit[["reddit", "query"]].copy().rename(columns={"reddit": "text"})
...
# %%
doc_topic_assignments.head()

# %%
topics_by_query.to_csv("../data/lda_topics_combined.csv", index=False)
doc_topic_assignments.to_csv("../data/lda_assignments_combined.csv", index=False)
